...
if page == "Compressive Strength":
    ...
    with col2:
        ...
        if st.button("Predict Durability", key="predict_v3", width='stretch'):
            ...
            f150_val   = float(resid_f150.predict(row)[0])  if resid_f150  else None
            f600_val   = float(resid_f600.predict(row)[0])  if resid_f600  else None
            fcrack_val = float(first_crack.predict(row)[0]) if first_crack  else None
            strain_val = float(strain_cap.predict(row)[0])  if strain_cap   else None
