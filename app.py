...
if page == "Compressive Strength":
    ...
    age_models = {
        ...
        28: None,  # use latest_result from V1
        56: load_optional_model(MODEL_CS_56DAY),
        90: load_optional_model(MODEL_CS_90DAY),
    }

    if st.session_state.latest_result and any(v is not None for k,v in age_models.items() if k != 28):
        inp = st.session_state.latest_result["inputs"]
        row_vals = {
            "Cement": inp.get("Cement",750), "Silica_Fume": inp.get("Silica Fume",150),
            "Fly_Ash": inp.get("Fly Ash",0), "Limestone": inp.get("Limestone Powder",0),
            "Quartz": inp.get("Quartz Powder",0), "Glass": 0, "Rice_Husk": 0,
            "Metakaolin": 0, "GGBFS": 0, "Slag": inp.get("Slag",0),
            "Nano_Silica": inp.get("Nano Silica",0), "Sand": inp.get("Aggregate",900),
            "Fiber": inp.get("Fibre",0), "Water": inp.get("Water",185),
            "SP": inp.get("Super Plasticizer",30),
        ...
        ages, strengths, r2s = [], [], []
        age_r2 = {1:0.83, 3:0.81, 7:0.79, 28:0.98, 56:0.68, 90:0.88}
        for age, m in age_models.items():
            if age == 28:
                strengths.append(st.session_state.latest_result["cs"])
            elif m is not None:
                strengths.append(float(m.predict(row_df)[0]))
            else:
                continue
            ages.append(age)
            r2s.append(age_r2[age])
