...
# ============================================================================
# FIBRE TYPE — interim calibration layer (added following beta tester feedback)
# ----------------------------------------------------------------------------
# The core model (model.pkl) was trained on a generic "Fibre" input and does
# NOT currently distinguish fibre type. In practice, steel fibre and
# synthetic/PVA/PP fibre produce materially different compressive strength
# outcomes at the same dosage, due to differences in micro-cracking behaviour,
# toughness, and matrix confinement.
#
# Until the model is retrained on fibre-type-labelled data, we apply a
# rule-based multiplicative correction to the raw prediction. This is an
# APPROXIMATION, not a learned relationship, and is clearly disclosed to
# the user in the UI and PDF report.
#
# Calibration basis (as of this update):
#   - Steel fibre: baseline (correction = 1.00), matches training data
#   - PVA / synthetic fibre: calibrated against real-world practitioner data
#     (30–40 kg/m3 PVA fibre, w/b 0.18–0.20 -> 100–120 MPa @ 28 days, vs a
#     142.3 MPa raw model prediction for the same mix treated as steel-type).
#     Correction factor = midpoint(100-120) / 142.3 ≈ 0.77
#   - Hybrid (steel + PVA): interpolated, no dedicated data point yet
#
# This is a SINGLE-SOURCE calibration point. Treat as indicative until more
# tester data is collected — do not present as a validated model.
# ============================================================================
FIBRE_TYPE_CORRECTION = {
    "Steel Fibre": 1.00,
    "PVA / Synthetic Fibre (PP)": 0.77,
    "Hybrid (Steel + PVA)": 0.89,
}

# Indicative GBP/kg unit rates — used in cost_calc(). Steel fibre rate is the
# original assumption; PVA/synthetic and hybrid are placeholder estimates
# pending real supplier quotes, flagged as indicative in the UI.
FIBRE_UNIT_COST = {
    "Steel Fibre": 1.75,
    "PVA / Synthetic Fibre (PP)": 3.20,
    "Hybrid (Steel + PVA)": 2.40,
}
...
MODEL_POROSITY   = "model_Porosity_pct.pkl"          # 239 rows, R²=0.73 (5-fold CV, retrained)
...
MODEL_CS_EXT     = "model_CS_extended.pkl"          # 2073 rows, R²=0.81

# Priority 1 — Post-crack performance
MODEL_RESID_F150 = "model_Residual_f150_MPa.pkl"    # 73 rows,  R²=0.95
MODEL_RESID_F600 = "model_Residual_f600_MPa.pkl"    # 68 rows,  R²=0.87
MODEL_FIRST_CRACK= "model_First_Crack_MPa.pkl"      # 103 rows, R²=0.89

# Priority 2 — Multi-age compressive strength
MODEL_CS_1DAY    = "model_CS_1day.pkl"              # 123 rows, R²=0.83
MODEL_CS_3DAY    = "model_CS_3day.pkl"              # 270 rows, R²=0.81
MODEL_CS_7DAY    = "model_CS_7day.pkl"              # 751 rows, R²=0.79
MODEL_CS_56DAY   = "model_CS_56day.pkl"             # 217 rows, R²=0.68
MODEL_CS_90DAY   = "model_CS_90day.pkl"             # 341 rows, R²=0.88

# Priority 3 — Ductility
MODEL_STRAIN_CAP = "model_Strain_Capacity_pct.pkl"  # 130 rows, R²=0.70

# Extended CS model feature order — must match exactly
CS_EXT_FEAT_COLS = [
    "Cement","Silica_Fume","Fly_Ash","Limestone","Quartz","Glass","Rice_Husk",
    "Metakaolin","GGBFS","Slag","Nano_Silica","Sand","Fiber","Water","SP",
    "Max_Agg_Size_mm","Curing_Temp_C","Curing_Humidity_pct",
]
CS_EXT_DEFAULTS = {
    "Max_Agg_Size_mm": 0.6,
    "Curing_Temp_C":   20.0,
    "Curing_Humidity_pct": 65.0,
}
...
def build_new_model_row(cement, sf, water, sp, fibre):
    """Single-row DataFrame for base 15-feature models."""
...
def build_extended_row(cement, sf, water, sp, fibre,
                       max_agg=0.6, cure_temp=20.0, humidity=65.0):
    """Single-row DataFrame for 18-feature models (includes curing conditions)."""
    values = {"Cement": cement, "Silica_Fume": sf, "Water": water, "SP": sp, "Fiber": fibre,
              "Max_Agg_Size_mm": max_agg, "Curing_Temp_C": cure_temp,
              "Curing_Humidity_pct": humidity, **NEW_MODEL_DEFAULTS}
    return pd.DataFrame([[values[c] for c in CS_EXT_FEAT_COLS]], columns=CS_EXT_FEAT_COLS)
...
AGE_MODEL_R2 = {1: 0.83, 3: 0.81, 7: 0.79, 28: 0.98, 56: 0.68, 90: 0.88}


def predict_age_curve(inputs, cs_28):
    """Predicted strength at 1/3/7/28/56/90 days using the real trained
    multi-age CS models (falls back to an indicative formula only if none
    of those model files are available). Returns (ages, strengths, r2s, is_real)."""
    age_models = {
        1:  load_optional_model(MODEL_CS_1DAY),
        3:  load_optional_model(MODEL_CS_3DAY),
        7:  load_optional_model(MODEL_CS_7DAY),
        28: None,  # always the already-computed base V1 prediction
        56: load_optional_model(MODEL_CS_56DAY),
        90: load_optional_model(MODEL_CS_90DAY),
    }
    has_real_models = any(m is not None for age, m in age_models.items() if age != 28)

    if not has_real_models:
        ages = np.array([1, 3, 7, 14, 28, 56, 90], dtype=float)
        strengths = cs_28 * (1 - np.exp(-ages / 18))
        r2s = [None] * len(ages)
        return list(ages), list(strengths), r2s, False

    row_vals = {
        "Cement": inputs.get("Cement", 750), "Silica_Fume": inputs.get("Silica Fume", 150),
        "Fly_Ash": inputs.get("Fly Ash", 0), "Limestone": inputs.get("Limestone Powder", 0),
        "Quartz": inputs.get("Quartz Powder", 0), "Glass": 0, "Rice_Husk": 0,
        "Metakaolin": 0, "GGBFS": 0, "Slag": inputs.get("Slag", 0),
        "Nano_Silica": inputs.get("Nano Silica", 0), "Sand": inputs.get("Aggregate", 900),
        "Fiber": inputs.get("Fibre", 0), "Water": inputs.get("Water", 185),
        "SP": inputs.get("Super Plasticizer", 30),
        "Max_Agg_Size_mm": 0.6, "Curing_Temp_C": 20.0, "Curing_Humidity_pct": 65.0,
    }
    row_df = pd.DataFrame([[row_vals[c] for c in CS_EXT_FEAT_COLS]], columns=CS_EXT_FEAT_COLS)

    ages, strengths, r2s = [], [], []
    for age, m in age_models.items():
        if age == 28:
            strengths.append(cs_28)
        elif m is not None:
            strengths.append(float(m.predict(row_df)[0]))
        else:
            continue
        ages.append(age)
        r2s.append(AGE_MODEL_R2[age])
    return ages, strengths, r2s, True
...
def cost_calc(inputs, fibre_type="Steel Fibre", mineral_filler_kg=0.0, sra_kg=0.0):
    cement, scm, water, sp, fibre = extract_materials(inputs)
    # Extract aggregate separately
    aggregate = 0.0
    for k, v in inputs.items():
        if "aggregate" in k.lower() and "nano" not in k.lower():
            aggregate += float(v)
    if aggregate == 0.0:
        aggregate = 900.0  # default aggregate content for UHPC if not specified

    fibre_rate = FIBRE_UNIT_COST.get(fibre_type, FIBRE_UNIT_COST["Steel Fibre"])

    # GBP/kg indicative unit rates (UK market 2025):
    # Cement:    £0.10/kg
    # SCM:       £0.55/kg (SF, slag, fly ash average)
    # Water:     £0.001/kg (negligible)
    # SP:        £4.00/kg
    # Fibre:     varies by type — see FIBRE_UNIT_COST
    # Aggregate: £0.15/kg (quartz sand/gravel)
    # Custom mineral filler (e.g. quartz powder): £0.60/kg indicative
    # SRA (shrinkage reducing admixture): £3.50/kg indicative
    material_cost = (0.10 * cement + 0.55 * scm + 0.001 * water +
                     4.00 * sp + fibre_rate * fibre + 0.15 * aggregate +
                     0.60 * mineral_filler_kg + 3.50 * sra_kg)
    # 25% overhead for mixing, placing, formwork (UHPC is labour-intensive)
    return material_cost * 1.25
...
def apply_fibre_correction(cs_raw, fibre_type):
    """Apply the interim fibre-type correction to a raw model prediction.
    Returns (corrected_cs, factor_used). See FIBRE_TYPE_CORRECTION notes above."""
    factor = FIBRE_TYPE_CORRECTION.get(fibre_type, 1.00)
    return cs_raw * factor, factor


# ============================================================================
# ROI ESTIMATOR (Beta) — indicative only
# ----------------------------------------------------------------------------
# Compares a UHPC mix against a conventional concrete baseline on a
# cost-per-structural-capacity and life-cycle basis. This is a simplified
# heuristic, NOT a structural design calculation — it does not account for
# actual member sizing, load cases, or code-governed design procedures.
# Intended to give engineers a directional sense of value, to be refined
# with a qualified structural engineer for any real project.
# ============================================================================
CONVENTIONAL_CONCRETE_COST_PER_M3 = 120.0   # GBP/m3, indicative C40 ready-mix
CONVENTIONAL_CONCRETE_STRENGTH_MPA = 40.0   # typical high-strength conventional concrete
CONVENTIONAL_SERVICE_LIFE_YEARS = 50
UHPC_SERVICE_LIFE_YEARS = 100
CONVENTIONAL_MAINTENANCE_PCT_PER_YEAR = 0.015  # % of initial cost, per year, indicative


def roi_calc(cs_mpa, uhpc_cost_per_m3):
    """Indicative ROI comparison of a UHPC mix vs conventional concrete.
    Returns a dict of comparison metrics. All figures are directional estimates."""
    if cs_mpa <= 0 or uhpc_cost_per_m3 <= 0:
        return None

    # Very rough volume/section-reduction heuristic: higher strength allows
    # thinner sections for the same load capacity. This uses a simple
    # strength-ratio proxy, not a real structural calc.
    strength_ratio = cs_mpa / CONVENTIONAL_CONCRETE_STRENGTH_MPA
    # Cap the assumed section reduction at a realistic 60% to avoid
    # unrealistic extrapolation at very high strengths
    volume_reduction_factor = max(0.4, min(1.0, 1.0 / (strength_ratio ** 0.5)))

    effective_uhpc_cost = uhpc_cost_per_m3 * volume_reduction_factor
    upfront_savings_pct = (CONVENTIONAL_CONCRETE_COST_PER_M3 - effective_uhpc_cost) / CONVENTIONAL_CONCRETE_COST_PER_M3 * 100

    # Life-cycle: conventional concrete typically needs replacement/major
    # rehab around its service life; UHPC's extended service life avoids
    # that recurring cost. Simplified to avoided maintenance cost over the
    # UHPC's service life horizon.
    conventional_annual_maintenance = CONVENTIONAL_CONCRETE_COST_PER_M3 * CONVENTIONAL_MAINTENANCE_PCT_PER_YEAR
    conventional_lifecycle_cost = (CONVENTIONAL_CONCRETE_COST_PER_M3 +
                                    conventional_annual_maintenance * UHPC_SERVICE_LIFE_YEARS)
    uhpc_lifecycle_cost = effective_uhpc_cost  # assume negligible maintenance over same horizon

    lifecycle_savings_pct = (conventional_lifecycle_cost - uhpc_lifecycle_cost) / conventional_lifecycle_cost * 100

    # Simple payback: if upfront cost is higher, how many years of avoided
    # maintenance does it take to break even
    upfront_premium = effective_uhpc_cost - CONVENTIONAL_CONCRETE_COST_PER_M3
    if upfront_premium > 0 and conventional_annual_maintenance > 0:
        payback_years = upfront_premium / conventional_annual_maintenance
    else:
        payback_years = 0.0

    return {
        "volume_reduction_pct": (1 - volume_reduction_factor) * 100,
        "effective_uhpc_cost_per_m3": effective_uhpc_cost,
        "conventional_cost_per_m3": CONVENTIONAL_CONCRETE_COST_PER_M3,
        "upfront_savings_pct": upfront_savings_pct,
        "conventional_lifecycle_cost": conventional_lifecycle_cost,
        "uhpc_lifecycle_cost": uhpc_lifecycle_cost,
        "lifecycle_savings_pct": lifecycle_savings_pct,
        "payback_years": payback_years,
        "uhpc_service_life": UHPC_SERVICE_LIFE_YEARS,
        "conventional_service_life": CONVENTIONAL_SERVICE_LIFE_YEARS,
    }
...
def recommendation_summary(inputs, cs, carbon, cost):
    ...
    expected = f"Estimated carbon: {carbon:.1f} kg CO2/m³ | Cost: {cost:.1f} GBP/m³"
    return recs, expected


def evaluate_mix(inputs, standard, fibre_type="Steel Fibre",
                  mineral_filler_note="", mineral_filler_kg=0.0, sra_kg=0.0):
    cs_raw = predict_strength(inputs)
    cs, fibre_correction_factor = apply_fibre_correction(cs_raw, fibre_type)
    E, ft, upv = derived_properties(cs, standard)
    carbon = carbon_calc(inputs)
    cost = cost_calc(inputs, fibre_type=fibre_type,
                      mineral_filler_kg=mineral_filler_kg, sra_kg=sra_kg)
    ...
    return {
        ...
        "fibre_correction_factor": fibre_correction_factor,
        "mineral_filler_note": mineral_filler_note,
        "mineral_filler_kg": mineral_filler_kg,
        "sra_kg": sra_kg,
        "ft": ft,
        "E": E,
        "upv": upv,
...
def build_pdf_chart(result, path):
    try:
        import kaleido  # noqa — optional dependency
        ...
        pass  # kaleido not installed — PDF chart section will be skipped gracefully


def generate_pdf(result, filename="AIcrete_Report.pdf", v2_result=None, v3_result=None, project_info=None):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # ── Project Decision (only if this report was generated from Project Creator) ──
        if project_info:
            verdict = project_info.get("verdict", "")
            verdict_colors = {
                "GO": (39, 174, 96), "REVIEW": (217, 140, 43), "STOP": (192, 57, 43),
            }
            verdict_labels = {
                "GO": "GO", "REVIEW": "REVIEW", "STOP": "STOP",
            }
            r, g, b = verdict_colors.get(verdict, (33, 38, 43))
            pdf.ln(8)
            pdf.set_font("Helvetica", "B", 12)
            pdf.cell(0, 8, pdf_safe("Project Decision"), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_font("Helvetica", "", 10)
            pdf.cell(0, 6, pdf_safe(
                f"Project: {project_info.get('name', 'N/A')}   |   "
                f"Location: {project_info.get('country', 'N/A') or 'N/A'}   |   "
                f"Structure: {project_info.get('structure', 'N/A')}"
            ), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(2)
            pdf.set_text_color(r, g, b)
            pdf.set_font("Helvetica", "B", 18)
            pdf.cell(0, 10, pdf_safe(verdict_labels.get(verdict, verdict)), new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.set_text_color(0, 0, 0)
            issues = project_info.get("issues") or []
            if issues:
                pdf.set_font("Helvetica", "", 10)
                for issue in issues:
                    pdf.multi_cell(0, 6, pdf_safe(f"- {issue}"))
                    pdf.set_x(pdf.l_margin)
            else:
                pdf.set_font("Helvetica", "", 10)
                pdf.multi_cell(0, 6, pdf_safe("This mix meets all project requirements entered in Project Creator."))
                pdf.set_x(pdf.l_margin)
        ...
        result_rows = [
            ...
            ("Cost per m3 (GBP indicative)", result["cost"]),
        ...
        if v2_result or v3_result:
            ...
            if v3_result:
                ...
                pdf.multi_cell(0, 5, pdf_safe(
                    ...
                    "Porosity R2 = 0.73, trained on 239 mixes."
                ))
                pdf.set_x(pdf.l_margin)
...
def shap_values(explainer, xdf):
    # check_additivity=False: RandomForestRegressor + SHAP can trip the internal
    # additivity check due to floating-point rounding in the tree ensemble sum,
    # even when the underlying explanation is valid. Disabling the check avoids
    # spurious failures that were previously falling back to a silent global
    # feature-importance chart with no warning to the user.
    vals = explainer.shap_values(xdf, check_additivity=False)
...
if "advisor_recs" not in st.session_state:
    st.session_state.advisor_recs = []


# ─────────────────────────────────────────────────────────────────────────────
# DESIGN ADVISOR — rule-based recommendation engine
# ─────────────────────────────────────────────────────────────────────────────
POROSITY_LIMITS = {
    "DC-1 (Dry)": 15, "DC-2 (Urban)": 12, "DC-3 (Wet)": 8,
    "DC-4 (Marine)": 5, "DC-4X (Salt Spray)": 4,
}

def generate_recommendations(result, v2=None, v3=None):
    recs = []
    cs      = result.get("cs", 0)
    carbon  = result.get("carbon", 0)
    score   = result.get("score", 0)
    compliance = result.get("compliance", [])

    # ── Compliance failures ──────────────────────────────────────────────────
    failing = [c["name"] for c in compliance if not c.get("ok", True)]
    if failing:
        recs.append({
            "type": "critical", "property": "Compliance",
            "issue": f"Fails minimum strength for: {', '.join(failing)}",
            "action": "Increase cement by 50 kg/m³ or silica fume by 20 kg/m³ to raise compressive strength.",
        })

    # ── Sustainability ───────────────────────────────────────────────────────
    if carbon > 450 and cs > 140:
        recs.append({
            "type": "opportunity", "property": "Embodied Carbon",
            "issue": f"High carbon ({carbon:.0f} kg CO₂/m³) relative to achieved strength ({cs:.0f} MPa)",
            "action": "Replace 80–100 kg/m³ cement with slag or fly ash — can reduce carbon by ~12–18% with minimal strength loss.",
        })

    if score < 40:
        recs.append({
            "type": "warning", "property": "Sustainability Score",
            "issue": f"Net Zero Concrete Score is low ({score:.0f}/100)",
            "action": "Use the Mix Optimiser to find a lower-carbon formulation that meets the same strength target.",
        })

    # ── V2 — Workability ─────────────────────────────────────────────────────
    if v2:
        slump = v2.get("slump_predicted", 200)
        if slump < 150:
            recs.append({
                "type": "critical", "property": "Workability",
                "issue": f"Poor workability ({slump:.0f} mm) — high placement and compaction risk",
                "action": "Increase superplasticiser by 5 kg/m³, or switch to CEM I 52.5 for better fresh-state flow.",
            })
        elif slump < 200:
            recs.append({
                "type": "warning", "property": "Workability",
                "issue": f"Moderate workability ({slump:.0f} mm) — may be challenging for complex formwork",
                "action": "Consider increasing SP dosage slightly or raising mixing temperature by 5°C.",
            })

    # ── V3 — Durability & Post-Crack ────────────────────────────────────────
    if v3:
        porosity    = v3.get("porosity", 0)
        design_life = v3.get("design_life", 0)
        exposure    = v3.get("exposure_class", "DC-3 (Wet)")
        fcrack      = v3.get("first_crack")
        f150        = v3.get("residual_f150")
        strain      = v3.get("strain_capacity")
        shrinkage   = v3.get("shrinkage")

        limit = POROSITY_LIMITS.get(exposure, 8)
        if porosity > limit:
            recs.append({
                "type": "critical", "property": "Porosity",
                "issue": f"Porosity {porosity:.1f}% exceeds limit ({limit}%) for {exposure}",
                "action": "Increase silica fume by 20–30 kg/m³ or reduce water by 10 kg/m³ to densify the matrix.",
            })

        if design_life < 50 and "DC-1" not in exposure:
            recs.append({
                "type": "warning", "property": "Design Life",
                "issue": f"Design life of {design_life:.0f} years may be insufficient for long-life infrastructure",
                "action": "Apply heat curing at 60°C — typically adds 15–20 years at no additional material cost.",
            })

        if fcrack and f150:
            ratio = f150 / fcrack if fcrack > 0 else 0
            if ratio < 1.1:
                recs.append({
                    "type": "warning", "property": "Post-Crack Behaviour",
                    "issue": f"Strain-softening detected (f150/fcrack = {ratio:.2f}) — load capacity drops after first crack",
                    "action": "Increase steel fibre by 20–30 kg/m³ to achieve strain-hardening behaviour (target ratio ≥ 1.1).",
                })

        if strain and strain < 0.5:
            recs.append({
                "type": "warning", "property": "Ductility",
                "issue": f"Low strain capacity ({strain:.2f}%) — limited ductility for seismic or blast design",
                "action": "Increase fibre dosage or consider a hybrid fibre system combining macro and micro fibres.",
            })

        if shrinkage and shrinkage > 300:
            recs.append({
                "type": "warning", "property": "Shrinkage",
                "issue": f"56-day shrinkage {shrinkage:.0f} µε exceeds 300 µε UHPC target",
                "action": "Reduce water content, increase aggregate volume, or add shrinkage-reducing admixture.",
            })

    if not recs:
        recs.append({
            "type": "success", "property": "All Clear",
            "issue": "No critical issues detected in this mix design",
            "action": "This mix meets predicted strength, durability, and compliance targets. Proceed to laboratory validation.",
        })

    return recs


def render_advisor(recs):
    colours = {
        "critical":    "#c0392b",
        "warning":     "#d98c2b",
        "opportunity": "#2f5870",
    ...
    icons = {"critical": "🔴", "warning": "🟡", "opportunity": "🔵", "success": "🟢"}
    st.markdown("### Design Advisor")
    st.caption("Automatic recommendations based on your predicted results — identify issues and opportunities before lab testing.")
    for r in recs:
        col = colours.get(r["type"], "#5b6470")
        icon = icons.get(r["type"], "⚪")
        st.markdown(f"""
        <div class="metric-card" style="border-left:3px solid {col};margin-bottom:0.75rem">
            <div class="metric-title">{icon} {r['property']}</div>
            <div style="font-size:0.9rem;color:#21262b;margin:0.3rem 0">{r['issue']}</div>
            <div style="font-size:0.85rem;color:{col};font-weight:600">→ {r['action']}</div>
        </div>""", unsafe_allow_html=True)


# ─────────────────────────────────────────────────────────────────────────────
# FIX MY MIX — targeted optimizer for a single failing property
# ─────────────────────────────────────────────────────────────────────────────
def fix_my_mix(result, target_property):
    """Return suggested input adjustments to fix a specific failing property."""
    inp = result.get("inputs", {})
    cement  = inp.get("Cement", 750)
    sf      = inp.get("Silica Fume", 150)
    water   = inp.get("Water", 185)
    sp      = inp.get("Super Plasticizer", 30)
    fibre   = inp.get("Fibre", 120)

    fixes = {
        "Compressive Strength": {
            "changes": {"Cement": +50, "Silica Fume": +20},
            "rationale": "Increasing cement and silica fume raises paste density and pozzolanic reaction, boosting CS by ~15–25 MPa.",
        },
        "Workability": {
            "changes": {"Super Plasticizer": +5, "Water": +5},
            "rationale": "Higher SP dosage improves flow without water-binder ratio penalty. Add water only if strength target allows.",
        },
        "Porosity": {
            "changes": {"Silica Fume": +25, "Water": -10},
            "rationale": "Silica fume fills capillary pores through pozzolanic reaction. Reducing water lowers the w/b ratio and densifies the matrix.",
        },
        "Post-Crack Behaviour": {
            "changes": {"Fibre": +25},
            "rationale": "Steel fibre dosage directly controls post-crack load-carrying capacity. +25 kg/m³ typically shifts f150/fcrack above 1.1.",
        },
        "Embodied Carbon": {
            "changes": {"Cement": -80, "Silica Fume": +30},
            "rationale": "Replacing cement with silica fume reduces embodied carbon significantly (SF has ~90% lower CO₂/kg than OPC) while maintaining strength.",
        },
        "Shrinkage": {
            "changes": {"Water": -15, "Silica Fume": +15},
            "rationale": "Lower water content and higher SF reduce autogenous and drying shrinkage in UHPC.",
        },
    }

    fix = fixes.get(target_property)
    if not fix:
        return None

    new_inputs = {
        "Cement": max(300, cement + fix["changes"].get("Cement", 0)),
        "Silica Fume": max(0, sf + fix["changes"].get("Silica Fume", 0)),
        "Water": max(120, water + fix["changes"].get("Water", 0)),
        "Super Plasticizer": max(0, sp + fix["changes"].get("Super Plasticizer", 0)),
        "Fibre": max(0, fibre + fix["changes"].get("Fibre", 0)),
    }
    return {"changes": fix["changes"], "new_inputs": new_inputs, "rationale": fix["rationale"]}
...
FAQ_BOT = [
    ...
    },
    {
        ...
        "answer": "All four prediction targets now use real trained models. V1 (Compressive Strength) R² = 0.98 on 810 mixes. V2 (Slump Flow) R² = 0.85 on 1,175 mixes (±14mm accuracy). V3 Flexural R² = 0.84, Split Tensile R² = 0.80, Porosity R² = 0.73, 56-day Shrinkage R² = 0.91, Toughness R² = 0.93. Full metrics on the Model Transparency page."
    },
    {
    ...
    },
    {
        ...
        "answer": "Durability & Service Life predicts flexural strength (R² = 0.84), split tensile strength (R² = 0.80), porosity (R² = 0.73), and estimated design/service life based on exposure class. All models are Production Ready."
    },
    {
    ...
    },
    {
        ...
        "answer": "The model was trained on peer-reviewed UHPC experimental literature and laboratory data. V2 (workability) is trained on 1,175 mixes and V3 (durability) properties on 140-239 mixes depending on the property, all drawn from a 2,188-mix master dataset compiled from 168 peer-reviewed sources."
    },
    {
    ...
    },
    {
        ...
        "answer": "Cost per m³ is estimated from material quantities × indicative GBP unit rates (UK market 2025). These are approximate only — local material prices vary significantly by region and supplier, so treat cost figures as indicative, not quotes."
    },
    {
...
[data-testid="stSidebar"] [role="radio"][aria-checked="true"] + div {
    color: #c084d8 !important;
}
/* Selected radio dot */
input[type="radio"]:checked + div svg {
    fill: #7B2D8B !important;
}
...
with st.sidebar:
    ...
    if "legal_page" in st.query_params:
        ...
        page = st.radio(
            "Navigation",
            ["Project Creator", "Compressive Strength", "Batch Upload", "Fresh-State Workability", "Durability & Service Life", "History", "Benchmarking", "Mix Optimizer", "Sensitivity Analysis", "SHAP Analysis", "Report", "Model Transparency"]
        )
...
def render_result_summary(result, show_save=True):
    ...
    with left:
        ...
        if result.get("fibre_correction_factor", 1.0) != 1.0:
            st.markdown(
                f'<div class="info-box" style="border-left:4px solid #f0a500;">'
                f'<strong>⚠ Fibre-type correction applied</strong><br/>'
                f'Raw model output (treats fibre generically): <strong>{result["cs_raw"]:.1f} MPa</strong><br/>'
                f'Adjusted for {result["fibre_type"]} '
                f'(×{result["fibre_correction_factor"]:.2f}, interim calibration — see Model Transparency page): '
                f'<strong>{result["cs"]:.1f} MPa</strong>'
                f'</div>',
                unsafe_allow_html=True
            )
        if result.get("mineral_filler_note") or result.get("sra_kg", 0) > 0:
            notes = []
            if result.get("mineral_filler_note"):
                notes.append(f'Custom filler "{result["mineral_filler_note"]}" ({result.get("mineral_filler_kg", 0):.0f} kg/m³) — included in cost, not yet in strength model.')
            if result.get("sra_kg", 0) > 0:
                notes.append(f'SRA {result["sra_kg"]:.1f} kg/m³ — included in cost, not yet in strength/durability model.')
        ...
        metrics = [
            ...
            ("Cost per m³", f'£{result["cost"]:.0f}', "GBP (indicative)"),
    ...
    with right:
        tabs = st.tabs(["Compliance", "Sustainability", "Interpretability", "Age Curve", "✅ Model Performance", "💰 ROI Estimator"])
        with tabs[0]:
        ...
        with tabs[2]:
            if shap is None:
                st.info("SHAP not installed. Showing model-wide feature importance instead (not specific to this mix).")
                if hasattr(MODEL, "feature_importances_"):
                    ...
                    fig = px.bar(imp, x="Feature", y="Importance", text="Importance", title="Feature Importance (Model-Wide)")
                ...
                try:
                    ...
                    fig = px.bar(local_df.head(10), x="Feature", y="SHAP Value", text="SHAP Value", title="Local SHAP Contribution (This Mix)")
                    fig.update_traces(texttemplate="%{text:.3f}", textposition="outside")
                    fig.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#21262b"))
                    st.plotly_chart(fig, width='stretch')
                except Exception as exc:
                    st.warning(
                        f"⚠ Could not generate a SHAP explanation for this specific mix ({exc}). "
                        f"Showing model-wide feature importance instead — this reflects the model's "
                        f"general behaviour, not this mix's individual prediction."
                    )
                    if hasattr(MODEL, "feature_importances_"):
                        imp = pd.DataFrame({"Feature": FEATURE_COLS, "Importance": MODEL.feature_importances_}).sort_values("Importance", ascending=False)
                        fig = px.bar(imp, x="Feature", y="Importance", text="Importance", title="Feature Importance (Model-Wide — Fallback)")
                        fig.update_traces(texttemplate="%{text:.3f}", textposition="outside")
                        fig.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#21262b"))
                        st.plotly_chart(fig, width='stretch')
        with tabs[3]:
            ages, strengths, r2s, is_real = predict_age_curve(result["inputs"], result["cs"])
            curve_df = pd.DataFrame({"Age": ages, "Strength": strengths})
            title = "Predicted Strength Development Curve" if is_real else "Indicative Age Curve (formula estimate)"
            fig = px.line(curve_df, x="Age", y="Strength", markers=True, title=title)
            fig.update_traces(
                line_color="#2f5870", line_width=3,
                marker=dict(size=9, color="#d98c2b", line=dict(width=2, color="#2f5870"))
            )
            fig.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#21262b"))
            st.plotly_chart(fig, width='stretch')
            if is_real:
                st.caption("AI-predicted using dedicated 1/3/7/56/90-day compressive strength models. 28-day uses the base prediction.")
            else:
                st.caption("Multi-age model files not found — showing a generic formula-based estimate, not an AI prediction. Upload model_CS_1day.pkl etc. alongside the app to enable real predictions here.")
        with tabs[4]:
        ...
        with tabs[5]:
            st.markdown("### ROI Estimator (Beta)")
            st.markdown(
                '<div class="info-box" style="border-left:4px solid #f0a500;">'
                '<strong>⚠ Indicative estimate only</strong> — this is a simplified heuristic '
                'comparing your mix to a generic conventional-concrete baseline (C40, £120/m³, '
                '50-year service life). It does not account for actual structural design, member '
                'sizing, or project-specific conditions. Use for directional guidance only; '
                'a qualified structural engineer should validate any real project figures.'
                '</div>', unsafe_allow_html=True
            )
            roi = result.get("roi")
            if roi:
                ...
                with rc1:
                    metric_card("Est. Section/Volume Reduction", f'{roi["volume_reduction_pct"]:.0f}%', "vs. conventional, same load case")
                with rc2:
                    metric_card("Upfront Cost vs Conventional",
                                f'{"+" if roi["upfront_savings_pct"] < 0 else "-"}{abs(roi["upfront_savings_pct"]):.0f}%',
                                "savings" if roi["upfront_savings_pct"] >= 0 else "premium")
                with rc3:
                    metric_card("Life-Cycle Savings", f'{roi["lifecycle_savings_pct"]:.0f}%',
                                f'over {roi["uhpc_service_life"]}yr horizon')

                st.markdown("---")
                comp_df = pd.DataFrame({
                    "Scenario": ["Conventional Concrete", "This UHPC Mix (effective, volume-adjusted)"],
                    "Upfront Cost (£/m³ equiv.)": [roi["conventional_cost_per_m3"], roi["effective_uhpc_cost_per_m3"]],
                    "Life-Cycle Cost (£/m³ equiv.)": [roi["conventional_lifecycle_cost"], roi["uhpc_lifecycle_cost"]],
                })
                st.dataframe(comp_df, width='stretch', hide_index=True)

                if roi["payback_years"] > 0:
                    st.markdown(
                        f'<div class="muted">If UHPC carries an upfront premium, estimated payback via avoided '
                        f'maintenance/replacement: <strong>~{roi["payback_years"]:.1f} years</strong> '
                        f'(against a {roi["conventional_service_life"]}-year conventional service life vs. '
                        f'{roi["uhpc_service_life"]}-year assumed UHPC service life).</div>',
                        unsafe_allow_html=True
                    )
                else:
                    st.markdown(
                        '<div class="muted">This mix is estimated to be cost-competitive upfront, '
                        'with additional life-cycle savings from reduced maintenance/replacement.</div>',
                        unsafe_allow_html=True
                    )

                st.caption(
                    f"Baseline assumptions: conventional concrete = {CONVENTIONAL_CONCRETE_STRENGTH_MPA:.0f} MPa, "
                    f"£{CONVENTIONAL_CONCRETE_COST_PER_M3:.0f}/m³, {CONVENTIONAL_MAINTENANCE_PCT_PER_YEAR*100:.1f}%/yr "
                    f"maintenance, {CONVENTIONAL_SERVICE_LIFE_YEARS}yr service life. Adjust these in code as your "
                    f"regional market data improves."
                )
            else:
                st.info("ROI estimate unavailable for this mix.")
...
# ─────────────────────────────────────────────────────────────────────────────
# PROJECT CREATOR — decision-first workflow
# ─────────────────────────────────────────────────────────────────────────────
STRUCTURE_DEFAULTS = {
    "Bridge & Infrastructure": {
        "exposure": "DC-4 (Marine)", "min_cs": 150, "min_life": 100,
        "cement": 800, "sf": 200, "water": 170, "sp": 35, "fibre": 156,
        "note": "Bridge decks require high durability and post-crack residual strength for fatigue resistance.",
    },
    "Building & Architecture": {
        "exposure": "DC-2 (Urban)", "min_cs": 120, "min_life": 50,
        "cement": 700, "sf": 150, "water": 185, "sp": 28, "fibre": 80,
        "note": "Architectural UHPC prioritises surface finish, flowability, and dimensional stability.",
    },
    "Industrial & Flooring": {
        "exposure": "DC-1 (Dry)", "min_cs": 130, "min_life": 30,
        "cement": 750, "sf": 160, "water": 180, "sp": 30, "fibre": 120,
        "note": "Industrial floors require high abrasion resistance and low shrinkage.",
    },
    "Precast & Prefab": {
        "exposure": "DC-2 (Urban)", "min_cs": 140, "min_life": 60,
        "cement": 780, "sf": 175, "water": 175, "sp": 32, "fibre": 140,
        "note": "Precast elements benefit from early strength for demoulding and heat curing compatibility.",
    },
    "Rehabilitation & Strengthening": {
        "exposure": "DC-3 (Wet)", "min_cs": 120, "min_life": 40,
        "cement": 720, "sf": 150, "water": 185, "sp": 28, "fibre": 100,
        "note": "Rehabilitation overlays require good bond, low shrinkage, and compatibility with existing substrate.",
    },
}

if page == "Project Creator":
    st.markdown('<div class="panel"><div class="panel-title">Project Creator</div><div class="panel-sub">Start with your project — AIcrete recommends a mix, runs strength, workability, durability, and post-crack performance models together, and gives you a clear GO / REVIEW / STOP decision.</div></div>', unsafe_allow_html=True)

    st.markdown("### Step 1 — Define Your Project")
    pc1, pc2 = st.columns(2)
    with pc1:
        proj_name     = st.text_input("Project Name", placeholder="e.g. Mauritius Bridge Deck Rehabilitation", key="proj_name")
        proj_country  = st.text_input("Country / Location", placeholder="e.g. Mauritius", key="proj_country")
        structure_type = st.selectbox("Structure Type", list(STRUCTURE_DEFAULTS.keys()), key="proj_struct")
    with pc2:
        defaults = STRUCTURE_DEFAULTS[structure_type]
        exposure  = st.selectbox("Service Exposure Class",
                                  ["DC-1 (Dry)", "DC-2 (Urban)", "DC-3 (Wet)", "DC-4 (Marine)", "DC-4X (Salt Spray)"],
                                  index=["DC-1 (Dry)", "DC-2 (Urban)", "DC-3 (Wet)", "DC-4 (Marine)", "DC-4X (Salt Spray)"].index(defaults["exposure"]),
                                  key="proj_exposure")
        target_cs     = st.slider("Target Compressive Strength (MPa)", 100, 200, defaults["min_cs"], key="proj_cs")
        target_life   = st.slider("Minimum Design Life (years)", 20, 150, defaults["min_life"], key="proj_life")
        carbon_target = st.slider("Max Embodied Carbon (kg CO₂/m³)", 200, 600, 450, key="proj_carbon")
        budget        = st.slider("Max Cost Budget (GBP/m³)", 300, 1500, 800, key="proj_budget")

    st.info(f"**{structure_type}** — {defaults['note']}")

    st.markdown("---")
    st.markdown("### Step 2 — Recommended Starting Mix")
    st.caption("Based on your project requirements. Adjust if needed before running predictions.")

    mc1, mc2, mc3 = st.columns(3)
    with mc1:
        rec_cement = st.slider("Cement (kg/m³)", 400, 1000, defaults["cement"], key="proj_rec_cement")
        rec_sf     = st.slider("Silica Fume (kg/m³)", 50, 300, defaults["sf"], key="proj_rec_sf")
    with mc2:
        rec_water  = st.slider("Water (kg/m³)", 130, 250, defaults["water"], key="proj_rec_water")
        rec_sp     = st.slider("SP (kg/m³)", 10, 60, defaults["sp"], key="proj_rec_sp")
    with mc3:
        rec_fibre  = st.slider("Steel Fibre (kg/m³)", 0, 300, defaults["fibre"], key="proj_rec_fibre")
        rec_std    = st.selectbox("Design Standard", ["Eurocode 2 (BS EN 1992-1-1)", "ACI 318", "AASHTO LRFD", "fib MC2010"], key="proj_std")

    st.markdown("---")
    st.markdown("### Step 3 — Run All Predictions")

    if st.button("🚀 Analyse Project Mix", key="proj_predict", width='stretch'):
        if not proj_name:
            st.warning("Please enter a project name first.")
        else:
            with st.spinner("Running all predictions..."):
                # Build inputs for V1
                proj_inputs = {
                    "Cement": rec_cement, "Slag": 0, "Silica Fume": rec_sf,
                    "Limestone Powder": 0, "Quartz Powder": 0, "Fly Ash": 0,
                    "Nano Silica": 0, "Aggregate": 900, "Water": rec_water,
                    "Fibre": rec_fibre, "Super Plasticizer": rec_sp,
                    "Temperature ": 20, "Age": 28,
                }
                proj_result = evaluate_mix(proj_inputs, rec_std)
                st.session_state.latest_result = proj_result
                st.session_state.shared_mix = {
                    "cement": rec_cement, "sf": rec_sf, "water": rec_water,
                    "sp": rec_sp, "fibre": rec_fibre,
                }

                # V2 slump
                _sm = load_optional_model(MODEL_SLUMP)
                if _sm is not None:
                    slump_row = build_slump_row(rec_cement, rec_sf, rec_water, rec_sp, rec_fibre)
                    slump_val = float(_sm.predict(slump_row)[0])
                else:
                    slump_val = 200 + (rec_sp-28)*8 - (rec_sf-150)*0.5
                    slump_val = max(100, min(350, slump_val))
                st.session_state.v2_result = {
                    "slump_predicted": slump_val, "mae": 14, "model_used": True,
                    "status": "Good Workability" if slump_val >= 250 else ("Moderate Workability" if slump_val >= 150 else "Poor Workability"),
                    "curing": "Ambient", "inputs": proj_inputs,
                }

                # V3 durability
                row     = build_new_model_row(rec_cement, rec_sf, rec_water, rec_sp, rec_fibre)
                row_ext = build_extended_row(rec_cement, rec_sf, rec_water, rec_sp, rec_fibre)
                _fm  = load_optional_model(MODEL_FLEXURAL_MOR)
                flexural = float(_fm.predict(row)[0]) if _fm else 20.0
                _pm  = load_optional_model(MODEL_POROSITY)
                porosity = float(_pm.predict(row)[0]) if _pm else 10.0
                _rm  = load_optional_model(MODEL_RESID_F150)
                f150 = float(_rm.predict(row_ext)[0]) if _rm else None
                _r6m = load_optional_model(MODEL_RESID_F600)
                f600 = float(_r6m.predict(row_ext)[0]) if _r6m else None
                _fcm = load_optional_model(MODEL_FIRST_CRACK)
                fcrack = float(_fcm.predict(row_ext)[0]) if _fcm else None
                _sc  = load_optional_model(MODEL_STRAIN_CAP)
                strain = float(_sc.predict(row_ext)[0]) if _sc else None
                _stm = load_optional_model(MODEL_SPLIT_T)
                split_tensile = float(_stm.predict(row)[0]) if _stm else None
                _shm = load_optional_model(MODEL_SHRINKAGE)
                shrinkage = float(_shm.predict(row)[0]) if _shm else None
                _tgm = load_optional_model(MODEL_TOUGHNESS)
                toughness = float(_tgm.predict(row)[0]) if _tgm else None
                exposure_base = {"DC-1 (Dry)": 30, "DC-2 (Urban)": 50, "DC-3 (Wet)": 75, "DC-4 (Marine)": 120, "DC-4X (Salt Spray)": 150}.get(exposure, 75)
                design_life = max(0, exposure_base * (1 - porosity / 25))
                st.session_state.v3_result = {
                    "flexural": flexural, "porosity": porosity, "design_life": design_life,
                    "first_crack": fcrack, "residual_f150": f150, "residual_f600": f600,
                    "strain_capacity": strain, "split_tensile": split_tensile,
                    "shrinkage": shrinkage, "toughness": toughness,
                    "exposure_class": exposure, "curing_time": 28,
                    "inputs": proj_inputs,
                }

                cs     = proj_result["cs"]
                carbon = proj_result["carbon"]
                cost   = proj_result["cost"]
                ...
                with r1c1:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Compressive Strength</div>
                        <div class="metric-value">{cs:.1f} MPa</div>
                        <div class="metric-sub">Target: {target_cs} MPa</div>
                    </div>""", unsafe_allow_html=True)
                with r1c2:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Slump Flow</div>
                        <div class="metric-value">{slump_val:.0f} mm</div>
                        <div class="metric-sub">{st.session_state.v2_result['status']}</div>
                    </div>""", unsafe_allow_html=True)
                with r1c3:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Embodied Carbon</div>
                        <div class="metric-value">{carbon:.0f} kg CO₂</div>
                        <div class="metric-sub">Target: &lt;{carbon_target}</div>
                    </div>""", unsafe_allow_html=True)
                with r1c4:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Cost (GBP/m³)</div>
                        <div class="metric-value">£{cost:.0f}</div>
                        <div class="metric-sub">Budget: £{budget}</div>
                    </div>""", unsafe_allow_html=True)

                r2c1, r2c2, r2c3, r2c4 = st.columns(4)
                with r2c1:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Flexural Strength</div>
                ...
                with r2c2:
                    st.markdown(f"""<div class="metric-card">
                        ...
                        <div class="metric-sub">R² = 0.73</div>
                    </div>""", unsafe_allow_html=True)
                with r2c3:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Design Life</div>
                        <div class="metric-value">{design_life:.0f} yrs</div>
                        <div class="metric-sub">{exposure}</div>
                    </div>""", unsafe_allow_html=True)
                with r2c4:
                    if f150 and fcrack:
                        ratio = f150/fcrack
                        behaviour = "Strain-Hardening" if ratio >= 1.1 else "Strain-Softening"
                        col = "#27ae60" if ratio >= 1.1 else "#c0392b"
                        st.markdown(f"""<div class="metric-card">
                            <div class="metric-title">Post-Crack Behaviour</div>
                            <div class="metric-value" style="color:{col};font-size:1.1rem">{behaviour}</div>
                            <div class="metric-sub">f150/fcrack = {ratio:.2f}</div>
                        </div>""", unsafe_allow_html=True)

                r3c1, r3c2, r3c3, r3c4 = st.columns(4)
                with r3c1:
                    if split_tensile is not None:
                        st.markdown(f"""<div class="metric-card">
                            <div class="metric-title">Split Tensile Strength</div>
                            <div class="metric-value">{split_tensile:.1f} MPa</div>
                            <div class="metric-sub">R² = 0.80</div>
                        </div>""", unsafe_allow_html=True)
                with r3c2:
                    if shrinkage is not None:
                        shrink_col = "#c0392b" if shrinkage > 800 else "#21262b"
                        st.markdown(f"""<div class="metric-card">
                            <div class="metric-title">56-Day Shrinkage</div>
                            <div class="metric-value" style="color:{shrink_col}">{shrinkage:.0f} microstrain</div>
                            <div class="metric-sub">R² = 0.91</div>
                        </div>""", unsafe_allow_html=True)
                with r3c3:
                    if toughness is not None:
                        st.markdown(f"""<div class="metric-card">
                            <div class="metric-title">Toughness</div>
                            <div class="metric-value">{toughness:.1f} J</div>
                            <div class="metric-sub">R² = 0.93</div>
                        </div>""", unsafe_allow_html=True)
                with r3c4:
                    if f600 is not None:
                        st.markdown(f"""<div class="metric-card">
                            <div class="metric-title">Residual Strength (f600)</div>
                            <div class="metric-value">{f600:.1f} MPa</div>
                            <div class="metric-sub">R² = 0.87</div>
                        </div>""", unsafe_allow_html=True)

                # ── Project Decision Verdict ─────────────────────────────────
                st.markdown("---")
                st.markdown("### Step 4 — Project Decision")

                issues = []
                if cs < target_cs:
                    issues.append(f"Compressive strength {cs:.0f} MPa below target {target_cs} MPa")
                if design_life < target_life:
                    issues.append(f"Design life {design_life:.0f} yrs below minimum {target_life} yrs")
                if carbon > carbon_target:
                    issues.append(f"Embodied carbon {carbon:.0f} kg CO₂/m³ exceeds target {carbon_target}")
                if cost > budget:
                    issues.append(f"Cost £{cost:.0f}/m³ exceeds budget £{budget}")
                if slump_val < 150:
                    issues.append(f"Poor workability ({slump_val:.0f} mm) — placement risk")
                if f150 and fcrack and f150/fcrack < 1.1:
                    issues.append("Strain-softening behaviour — increase fibre content")
                if shrinkage is not None and shrinkage > 800 and exposure in ("DC-3 (Wet)", "DC-4 (Marine)", "DC-4X (Salt Spray)"):
                    issues.append(f"High 56-day shrinkage ({shrinkage:.0f} microstrain) raises cracking risk in {exposure} exposure")

                if len(issues) == 0:
                    st.markdown(f"""<div style="background:#e6f4ea;border:2px solid #27ae60;border-radius:6px;padding:1.5rem;text-align:center">
                        <div style="font-family:'Oswald',sans-serif;font-size:2.5rem;font-weight:700;color:#27ae60">✅ GO</div>
                        <div style="font-size:1rem;color:#21262b;margin-top:0.5rem">This mix meets all project requirements for <strong>{proj_name}</strong>.</div>
                        <div style="font-size:0.9rem;color:#5b6470;margin-top:0.3rem">Proceed to laboratory validation before implementation.</div>
                    </div>""", unsafe_allow_html=True)
                elif len(issues) <= 2:
                    issues_html = "".join([f"<li>{i}</li>" for i in issues])
                    st.markdown(f"""<div style="background:#fff8e6;border:2px solid #d98c2b;border-radius:6px;padding:1.5rem;text-align:center">
                        <div style="font-family:'Oswald',sans-serif;font-size:2.5rem;font-weight:700;color:#d98c2b">⚠️ REVIEW</div>
                        <div style="font-size:1rem;color:#21262b;margin-top:0.5rem">Mix needs adjustment before proceeding for <strong>{proj_name}</strong>.</div>
                        <ul style="text-align:left;margin-top:0.75rem;color:#5b6470">{issues_html}</ul>
                        <div style="font-size:0.9rem;color:#5b6470">Use Fix My Mix on the Compressive Strength page to address these issues.</div>
                    </div>""", unsafe_allow_html=True)
                else:
                    issues_html = "".join([f"<li>{i}</li>" for i in issues])
                    st.markdown(f"""<div style="background:#fde8e8;border:2px solid #c0392b;border-radius:6px;padding:1.5rem;text-align:center">
                        <div style="font-family:'Oswald',sans-serif;font-size:2.5rem;font-weight:700;color:#c0392b">🛑 STOP</div>
                        <div style="font-size:1rem;color:#21262b;margin-top:0.5rem">This mix does not meet project requirements for <strong>{proj_name}</strong>.</div>
                        <ul style="text-align:left;margin-top:0.75rem;color:#5b6470">{issues_html}</ul>
                        <div style="font-size:0.9rem;color:#5b6470">Significant mix redesign required. Use the Mix Optimizer to find a compliant formulation.</div>
                    </div>""", unsafe_allow_html=True)

                # Save to session for report — use prefixed keys to avoid widget key conflicts
                st.session_state["_proj_name"]      = proj_name
                st.session_state["_proj_country"]   = proj_country
                st.session_state["_proj_structure"] = structure_type
                st.session_state["_proj_verdict"]   = "GO" if len(issues) == 0 else ("REVIEW" if len(issues) <= 2 else "STOP")
                st.session_state["_proj_issues"]    = issues

                # Design Advisor
                st.markdown("---")
                recs = generate_recommendations(proj_result, v2=st.session_state.v2_result, v3=st.session_state.v3_result)
                render_advisor(recs)

                st.success("✓ Results saved — go to Report to generate a full project PDF.")
...
if page == "Compressive Strength":
    ...
    with left:
        ...
        st.markdown('<div class="panel-title" style="margin-top:0.8rem;">Fibre & Filler Details</div>', unsafe_allow_html=True)
        fibre_type = st.selectbox(
            "Fibre Type",
            list(FIBRE_TYPE_CORRECTION.keys()),
            key="pred_fibre_type",
            help="The core model treats fibre generically. This applies an interim, "
                 "single-data-point calibration correction based on beta tester feedback — "
                 "see About page for details. Not yet a fully retrained model."
        )
        col_f1, col_f2 = st.columns(2)
        with col_f1:
            mineral_filler_note = st.text_input(
                "Custom Mineral Filler (optional)",
                "",
                key="pred_filler_name",
                help="e.g. quartz powder, calcined clay. Recorded for your reference and "
                     "included in cost — not yet factored into the strength prediction."
            )
        with col_f2:
            mineral_filler_kg = st.number_input(
                "Filler Dosage (kg/m³)", 0.0, 300.0, 0.0, step=1.0, key="pred_filler_kg"
            )
        sra_kg = st.number_input(
            "SRA — Shrinkage Reducing Admixture (kg/m³, optional)",
            0.0, 20.0, 0.0, step=0.1, key="pred_sra_kg",
            help="Recorded and included in cost estimate — not yet factored into strength/durability prediction."
        )

        if st.button("Predict Strength", width='stretch'):
            result = evaluate_mix(inputs, standard, fibre_type=fibre_type,
                                   mineral_filler_note=mineral_filler_note,
                                   mineral_filler_kg=mineral_filler_kg, sra_kg=sra_kg)
            st.session_state.latest_result = result
    ...
    if st.session_state.latest_result:
        ...
        recs = generate_recommendations(
            ...
            v3=st.session_state.v3_result,
        )
        st.session_state.advisor_recs = recs
        render_advisor(recs)

        # ── Fix My Mix ────────────────────────────────────────────────────────
        st.markdown("---")
        st.markdown('<div class="panel"><div class="panel-title">Fix My Mix</div><div class="panel-sub">Select a property to improve — get targeted mix adjustments instantly.</div></div>', unsafe_allow_html=True)

        fixable = ["Compressive Strength", "Workability", "Porosity",
                   "Post-Crack Behaviour", "Embodied Carbon", "Shrinkage"]
        fix_target = st.selectbox("Which property do you want to improve?", fixable, key="fix_target")

        if st.button("Get Fix", key="fix_btn", width='stretch'):
            fix = fix_my_mix(st.session_state.latest_result, fix_target)
            if fix:
                st.markdown(f"#### Suggested adjustments to improve **{fix_target}**")
                fc1, fc2 = st.columns(2)
                with fc1:
                    st.markdown("**Changes to apply:**")
                    for param, delta in fix["changes"].items():
                        arrow = "▲" if delta > 0 else "▼"
                        col = "#27ae60" if delta > 0 else "#c0392b"
                        st.markdown(f"""<div style="font-size:0.95rem;padding:0.3rem 0">
                            <span style="color:{col};font-weight:700">{arrow} {abs(delta)} kg/m³</span>
                            <span style="color:#21262b"> {param}</span>
                        </div>""", unsafe_allow_html=True)
                with fc2:
                    st.markdown("**New suggested values:**")
                    for param, val in fix["new_inputs"].items():
                        st.markdown(f"""<div style="font-size:0.9rem;padding:0.2rem 0;color:#5b6470">
                            {param}: <strong style="color:#2f5870">{val:.0f} kg/m³</strong>
                        </div>""", unsafe_allow_html=True)
                st.info(f"**Why this works:** {fix['rationale']}")
                st.caption("Run a new prediction with these adjusted values to see the updated result.")

    # ── Carbon Trade-off Visualiser ───────────────────────────────────────────
    if st.session_state.latest_result:
        st.markdown("---")
        st.markdown('<div class="panel"><div class="panel-title">Carbon Trade-off Visualiser</div><div class="panel-sub">See how different silica fume and cement combinations shift strength vs embodied carbon — find the low-carbon sweet spot.</div></div>', unsafe_allow_html=True)

        inp = st.session_state.latest_result["inputs"]
        base_water  = inp.get("Water", 185)
        base_sp     = inp.get("Super Plasticizer", 30)
        base_fibre  = inp.get("Fibre", 120)
        base_agg    = inp.get("Aggregate", 900)

        viz_points = []
        model_v1   = load_model()
        for cement in range(450, 1000, 75):
            for sf in range(50, 300, 50):
                try:
                    row_inp = {
                        "Cement": cement, "Slag": 0, "Silica Fume": sf,
                        "Limestone Powder": 0, "Quartz Powder": 0, "Fly Ash": 0,
                        "Nano Silica": 0, "Aggregate": base_agg,
                        "Water": base_water, "Fibre": base_fibre,
                        "Super Plasticizer": base_sp, "Temperature ": 20, "Age": 28,
                    }
                    row_df = pd.DataFrame([row_inp])
                    cs_pred = float(model_v1.predict(row_df)[0])
                    co2 = 0.90 * cement + 0.12 * sf + 0.0003 * base_water + 0.08 * base_fibre
                    viz_points.append({"CS": round(cs_pred, 1), "Carbon": round(co2, 0),
                                       "Cement": cement, "SF": sf})
                except Exception:
                    pass
        ...
        if viz_points:
            ...
            fig = px.scatter(
                viz_df, x="Carbon", y="CS",
                color="SF", size_max=10,
                color_continuous_scale=[[0,"#8fafc0"],[0.5,"#2f5870"],[1,"#d98c2b"]],
                labels={"Carbon": "Embodied Carbon (kg CO₂/m³)", "CS": "Predicted CS (MPa)", "SF": "SF (kg/m³)"},
                title="Strength vs Embodied Carbon — Mix Design Space",
                hover_data={"Cement": True, "SF": True},
            )
            # Mark current mix
            curr_cs     = st.session_state.latest_result["cs"]
            curr_carbon = st.session_state.latest_result["carbon"]
            fig.add_scatter(
                x=[curr_carbon], y=[curr_cs], mode="markers",
                marker=dict(size=14, color="#d98c2b", symbol="star", line=dict(width=2, color="#21262b")),
                name="Your mix",
            )
            fig.update_layout(
                plot_bgcolor="white", paper_bgcolor="white",
                font=dict(family="Inter, sans-serif", color="#21262b"),
                xaxis=dict(gridcolor="#efece6"),
                yaxis=dict(gridcolor="#efece6"),
                coloraxis_colorbar=dict(title="SF kg/m³"),
            )
            st.plotly_chart(fig)
            st.caption("★ = your current mix. Each point is a different cement/silica fume combination. Higher SF (amber) generally means lower carbon for the same strength.")

    # ── Extended Prediction Panel (curing + aggregate size) ──────────────────
    st.markdown("---")
    st.markdown('<div class="panel"><div class="panel-title">Extended Strength Prediction</div><div class="panel-sub">Add curing conditions and aggregate size for a higher-accuracy prediction — R² = 0.81 on 2,073 mixes.</div></div>', unsafe_allow_html=True)

    cs_ext_model = load_optional_model(MODEL_CS_EXT)
    if cs_ext_model is None:
        st.warning(f"Extended model file ({MODEL_CS_EXT}) not found — upload it alongside the app to enable this feature.")
    else:
        st.info("**Extended Model** | R² = 0.81 | 2,073 mixes | Includes curing temperature (26% importance), max aggregate size, and humidity")
        ec1, ec2, ec3 = st.columns(3)
        with ec1:
            ext_cure_temp = st.slider("Curing Temperature (°C)", 20, 210, 20, key="ext_cure_temp",
                                      help="Standard curing: 20°C. Heat curing: 60-90°C. Steam/autoclave: 100-210°C.")
        with ec2:
            ext_max_size  = st.slider("Max Aggregate Size (mm)", 0.1, 4.8, 0.6, step=0.1, key="ext_max_size",
                                      help="Typical UHPC: 0.3-0.6mm quartz sand. Some mixes use up to 2-4.75mm.")
        with ec3:
            ext_humidity  = st.slider("Curing Humidity (%)", 50, 100, 65, key="ext_humidity",
                                      help="Standard lab: 65%. Moist/sealed curing: 95-100%.")

        if st.button("Run Extended Prediction", key="ext_predict", width='stretch'):
            if st.session_state.latest_result:
                inp = st.session_state.latest_result["inputs"]
                row_vals = {
                    "Cement":       inp.get("Cement", 750),
                    "Silica_Fume":  inp.get("Silica Fume", 150),
                    "Fly_Ash":      inp.get("Fly Ash", 0),
                    "Limestone":    inp.get("Limestone Powder", 0),
                    "Quartz":       inp.get("Quartz Powder", 0),
                    "Glass":        inp.get("Glass", 0),
                    "Rice_Husk":    inp.get("Rice Husk", 0),
                    "Metakaolin":   inp.get("Metakaolin", 0),
                    "GGBFS":        inp.get("GGBFS", 0),
                    "Slag":         inp.get("Slag", 0),
                    "Nano_Silica":  inp.get("Nano Silica", 0),
                    "Sand":         inp.get("Aggregate", 900),
                    "Fiber":        inp.get("Fibre", 0),
                    "Water":        inp.get("Water", 185),
                    "SP":           inp.get("Super Plasticizer", 30),
                    "Max_Agg_Size_mm":    ext_max_size,
                    "Curing_Temp_C":      ext_cure_temp,
                    "Curing_Humidity_pct": ext_humidity,
                }
                row_df = pd.DataFrame([[row_vals[c] for c in CS_EXT_FEAT_COLS]], columns=CS_EXT_FEAT_COLS)
                ext_cs = float(cs_ext_model.predict(row_df)[0])
                base_cs = st.session_state.latest_result["cs"]

                xc1, xc2, xc3 = st.columns(3)
                with xc1:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Extended CS Prediction (MPa)</div>
                        <div class="metric-value">{ext_cs:.1f} MPa</div>
                        <div class="metric-sub">R² = 0.81 | Includes curing conditions</div>
                    </div>""", unsafe_allow_html=True)
                with xc2:
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Standard Prediction (MPa)</div>
                        <div class="metric-value">{base_cs:.1f} MPa</div>
                        <div class="metric-sub">R² = 0.98 | Mix proportions only</div>
                    </div>""", unsafe_allow_html=True)
                with xc3:
                    diff = ext_cs - base_cs
                    direction = "higher" if diff > 0 else "lower"
                    st.markdown(f"""<div class="metric-card">
                        <div class="metric-title">Curing Effect</div>
                        <div class="metric-value" style="color:{'#2f5870' if diff>0 else '#c0392b'}">{diff:+.1f} MPa</div>
                        <div class="metric-sub">{abs(diff):.1f} MPa {direction} with these curing conditions</div>
                    </div>""", unsafe_allow_html=True)

                if ext_cure_temp > 60:
                    st.success(f"Elevated curing at {ext_cure_temp}°C contributes significantly — UHPC strength increases substantially with heat/steam curing.")
                elif ext_cure_temp == 20:
                    st.info("Standard ambient curing (20°C). Try increasing temperature to see the effect of heat/steam curing on strength.")
            else:
                st.warning("Run a standard prediction first, then use Extended Prediction to layer in curing conditions.")

    # ── Strength Development Curve ───────────────────────────────────────────
    st.markdown("---")
    st.markdown('<div class="panel"><div class="panel-title">Strength Development Curve</div><div class="panel-sub">Predicted compressive strength at 1, 3, 7, 28, 56 and 90 days — useful for construction programme planning and demoulding decisions.</div></div>', unsafe_allow_html=True)

    if st.session_state.latest_result:
        inp = st.session_state.latest_result["inputs"]
        cs_28 = st.session_state.latest_result["cs"]
        ages, strengths, r2s, is_real = predict_age_curve(inp, cs_28)

        if is_real and len(ages) >= 3:
            fig = px.line(
                x=ages, y=strengths,
                markers=True,
                labels={"x": "Age (days)", "y": "Compressive Strength (MPa)"},
                title="Predicted Strength Development Curve",
            )
            fig.update_traces(
                line_color="#2f5870", line_width=3,
                marker=dict(size=10, color="#d98c2b", line=dict(width=2, color="#2f5870"))
            )
            fig.update_layout(
                plot_bgcolor="white", paper_bgcolor="white",
                font=dict(family="Inter, sans-serif", color="#21262b"),
                xaxis=dict(tickvals=ages, gridcolor="#efece6"),
                yaxis=dict(gridcolor="#efece6"),
                title_font_size=14,
            )
            # Add 28-day annotation
            fig.add_annotation(x=28, y=cs_28, text=f"28d: {cs_28:.0f} MPa",
                               showarrow=True, arrowhead=2, ax=30, ay=-30,
                               font=dict(color="#d98c2b", size=11))
            st.plotly_chart(fig)

            # Summary table
            cols_age = st.columns(len(ages))
            for i, (age, cs, r2) in enumerate(zip(ages, strengths, r2s)):
                with cols_age[i]:
                    st.markdown(f"""<div class="metric-card" style="text-align:center">
                        <div class="metric-title">{age}-Day</div>
                        <div class="metric-value" style="font-size:1.2rem">{cs:.0f} MPa</div>
                        <div class="metric-sub">R²={r2:.2f}</div>
                    </div>""", unsafe_allow_html=True)
        elif not is_real:
            st.info("Multi-age model files (model_CS_1day.pkl, etc.) not found alongside the app — upload them to enable this curve.")
    else:
        st.info("Run a Compressive Strength prediction first to generate the strength development curve.")

elif page == "Batch Upload":
    st.markdown(
        '<div class="panel"><div class="panel-title">Batch Mix Design Upload</div>'
        '<div class="panel-sub">Upload an Excel or CSV file with multiple mix designs and get '
        'predictions for all of them at once — no need to enter each mix manually.</div></div>',
        unsafe_allow_html=True
    )

    st.markdown("### 1. Download the template")
    template_df = pd.DataFrame([{col: RANGES[col]["mean"] for col in FEATURE_COLS}])
    template_df.insert(0, "Mix Name", "Mix 1")
    template_df["Fibre Type"] = "Steel Fibre"
    template_buf = BytesIO()
    with pd.ExcelWriter(template_buf, engine="openpyxl") as writer:
        template_df.to_excel(writer, index=False, sheet_name="Mixes")
    st.download_button(
        "📥 Download Excel Template",
        data=template_buf.getvalue(),
        file_name="AIcrete_Batch_Template.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
    st.caption(
        f"Columns expected: Mix Name, {', '.join(FEATURE_COLS)}, and optionally 'Fibre Type' "
        f"(Steel Fibre / PVA / Synthetic Fibre (PP) / Hybrid (Steel + PVA)). "
        f"Any missing values are filled with the model's training-data average for that ingredient."
    )

    st.markdown("### 2. Upload your file")
    batch_standard = st.selectbox("Design Standard (applied to all rows)", STANDARD_OPTIONS, key="batch_std")
    uploaded = st.file_uploader("Excel (.xlsx) or CSV file", type=["xlsx", "csv"], key="batch_uploader")
    ...
    if uploaded is not None:
        ...
        if batch_df is not None and len(batch_df) > 0:
            # Case-insensitive column matching against expected FEATURE_COLS
            col_lookup = {c.strip().lower(): c for c in batch_df.columns}
            matched = {}
            missing = []
            for feat in FEATURE_COLS:
                key = feat.strip().lower()
                if key in col_lookup:
                    matched[feat] = col_lookup[key]
                else:
                    missing.append(feat)

            name_col = None
            for candidate in ["mix name", "name", "session name", "mix"]:
                if candidate in col_lookup:
                    name_col = col_lookup[candidate]
                    break

            fibre_type_col = col_lookup.get("fibre type")

            if missing:
                st.warning(
                    f"These columns were not found and will use the training-data average for every row: "
                    f"{', '.join(missing)}"
                )
            ...
            if st.button("Run Batch Predictions", width='stretch'):
                ...
                for i, (_, row) in enumerate(batch_df.iterrows()):
                    ...
                    for feat in FEATURE_COLS:
                        if feat in matched:
                            try:
                                row_inputs[feat] = float(row[matched[feat]])
                            except Exception:
                                row_inputs[feat] = RANGES[feat]["mean"]
                        else:
                            row_inputs[feat] = RANGES[feat]["mean"]

                    row_fibre_type = "Steel Fibre"
                    if fibre_type_col is not None:
                        raw_ft = str(row.get(fibre_type_col, "")).strip()
                        for ft_key in FIBRE_TYPE_CORRECTION.keys():
                            if raw_ft.lower() in ft_key.lower() or ft_key.lower() in raw_ft.lower():
                                row_fibre_type = ft_key
                                break

                    mix_name = str(row[name_col]) if name_col else f"Mix {i+1}"

                    try:
                        res = evaluate_mix(row_inputs, batch_standard, fibre_type=row_fibre_type)
                        roi = res.get("roi") or {}
                        results_rows.append({
                            "Mix Name": mix_name,
                            "Fibre Type": row_fibre_type,
                            "Predicted CS (MPa)": round(res["cs"], 1),
                            "Tensile Strength (MPa)": round(res["ft"], 2),
                            "Elastic Modulus (GPa)": round(res["E"], 2),
                            "Embodied Carbon (kg CO2/m3)": round(res["carbon"], 1),
                            "Cost (£/m3)": round(res["cost"], 0),
                            "Sustainability Score": round(res["score"], 0),
                            "Confidence": res["confidence_label"],
                            "Upfront Savings vs Conventional (%)": round(roi.get("upfront_savings_pct", 0), 0) if roi else None,
                        })
                    except Exception as e:
                        results_rows.append({
                            "Mix Name": mix_name, "Fibre Type": row_fibre_type,
                            "Predicted CS (MPa)": None, "Error": str(e)
                        })
                    progress.progress((i + 1) / n, text=f"Running predictions... ({i+1}/{n})")
                ...
                st.download_button(
                    ...
                    file_name="AIcrete_Batch_Results.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                )
                st.caption(
                    "⚠ All predictions above are preliminary engineering estimates. Laboratory validation "
                    "and professional engineering review are required before use in any construction project."
                )
    ...
    resid_f150    = load_optional_model(MODEL_RESID_F150)
    resid_f600    = load_optional_model(MODEL_RESID_F600)
    first_crack   = load_optional_model(MODEL_FIRST_CRACK)
    strain_cap    = load_optional_model(MODEL_STRAIN_CAP)
    ...
    with col2:
        ...
        cure_temp_v3   = st.slider("Curing Temperature (°C)", 20, 210, 20, key="v3_cure_temp",
                                   help="Standard: 20°C. Heat curing: 60-90°C. Steam: 100-210°C.")
        humidity_v3    = st.slider("Humidity (%)", 50, 100, 65, key="v3_humidity")
        st.markdown("&nbsp;")
        ...
        if st.button("Predict Durability", key="predict_v3", width='stretch'):
            ...
            row_ext  = build_extended_row(cement_v3, sf_v3, water_v3, sp_v3, fibre_v3,
                                          max_agg=0.6, cure_temp=cure_temp_v3, humidity=humidity_v3)
            ...
            # Post-crack performance — needs extended row (18 features)
            f150_val   = float(resid_f150.predict(row_ext)[0])  if resid_f150  else None
            f600_val   = float(resid_f600.predict(row_ext)[0])  if resid_f600  else None
            fcrack_val = float(first_crack.predict(row_ext)[0]) if first_crack  else None
            strain_val = float(strain_cap.predict(row_ext)[0])  if strain_cap   else None
            ...
            st.session_state.v3_result = {
                ...
                "porosity": porosity, "design_life": design_life,
                "shrinkage": shrinkage, "toughness": toughness,
                "residual_f150": f150_val, "residual_f600": f600_val,
                "first_crack": fcrack_val, "strain_capacity": strain_val,
            ...
            # Post-crack performance section
            if any(v is not None for v in [f150_val, f600_val, fcrack_val, strain_val]):
                st.markdown("#### Post-Crack Performance")
                st.caption("These properties define UHPC's structural response beyond the elastic range — critical for seismic, blast, and impact design.")
                pc_cols = [c for c in [
                    ("First Cracking Strength (MPa)", fcrack_val, "R² = 0.89", "Load at which micro-cracking initiates"),
                    ("Residual Strength f150 (MPa)", f150_val, "R² = 0.95", "Post-crack capacity at δ/L = 1/150"),
                    ("Residual Strength f600 (MPa)", f600_val, "R² = 0.87", "Post-crack capacity at δ/L = 1/600"),
                    ("Strain Capacity (%)", strain_val, "R² = 0.70", "Ultimate tensile strain — ductility indicator"),
                ] if c[1] is not None]

                cols = st.columns(len(pc_cols))
                for i, (label, val, r2_txt, note) in enumerate(pc_cols):
                    with cols[i]:
                        st.markdown(f"""<div class="metric-card">
                            <div class="metric-title">{label}</div>
                            <div class="metric-value">{val:.2f}</div>
                            <div class="metric-sub">{r2_txt} | {note}</div>
                        </div>""", unsafe_allow_html=True)

                # Post-crack response interpretation
                if f150_val and fcrack_val:
                    ratio = f150_val / fcrack_val if fcrack_val > 0 else 0
                    if ratio >= 1.1:
                        st.success(f"Strain-hardening behaviour (f150/fcrack = {ratio:.2f}) — this mix exhibits true UHPC ductility after cracking.")
                    else:
                        st.warning(f"Strain-softening behaviour (f150/fcrack = {ratio:.2f}) — consider increasing fibre content for better post-crack ductility.")
...
| **V3 — Durability** | Flexural strength, split tensile, porosity, design life | ✅ Production Ready | Peak Flexural R² = 0.72 (1,024 mixes), MOR Flexural R² = 0.84 (140 mixes), Split Tensile R² = 0.80 (237 mixes), Porosity R² = 0.73 (239 mixes) |
...
- **Version 3 (Durability):** ✅ Production Ready — flexural strength (R² = 0.84), split tensile (R² = 0.80), porosity (R² = 0.73), design life estimation
...
We welcome technical feedback from engineers and researchers. Please use the feedback form or contact us directly.
    ...
    if "sensitivity_df" in st.session_state:
        ...
        with c2:
            ...
            st.plotly_chart(fig, width='stretch')
        with c3:
            fig = px.line(sdf, x=svar, y="Cost", title="Cost per m³ (GBP)")
            fig.update_layout(paper_bgcolor="white", plot_bgcolor="white", font=dict(color="#21262b"))
            st.plotly_chart(fig, width='stretch')
    ...
    if source is None:
        ...
        has_project = bool(st.session_state.get("_proj_verdict"))
        if has_project:
            include_project = st.checkbox(
                f"Include Project Decision — {st.session_state.get('_proj_name', 'Untitled Project')} "
                f"({st.session_state.get('_proj_verdict')})",
                value=True,
            )
        else:
            st.caption("Run a project through Project Creator to include its GO / REVIEW / STOP decision in this report.")
        ...
        with c1:
            if st.button("Generate Report", width='stretch'):
                try:
                    ...
                    if include_project:
                        project_info = {
                            "name": st.session_state.get("_proj_name"),
                            "country": st.session_state.get("_proj_country"),
                            "structure": st.session_state.get("_proj_structure"),
                            "verdict": st.session_state.get("_proj_verdict"),
                            "issues": st.session_state.get("_proj_issues"),
                        }
                    path = generate_pdf(
                        source, file_name,
...
### 9. Contact
For any questions regarding these Terms of Service, please contact us at team.aicrete@gmail.com
    """)
...
### 11. Contact Us
For privacy inquiries, contact: team.aicrete@gmail.com
    """)
...
If you discover a security vulnerability, please report it to: team.aicrete@gmail.com
...
### 9. Contact
For security concerns, contact: team.aicrete@gmail.com
    """)
        ...
        <div class="footer-main">
            <div class="footer-column">
                ...
                <div class="footer-links">
                    ...
                    <a href="https://aicretesolutions.co.uk" class="footer-link" target="_blank">Documentation</a>
                    <a href="https://aicretesolutions.co.uk" class="footer-link" target="_blank">API</a>
                    <a href="https://aicretesolutions.co.uk" class="footer-link" target="_blank">Pricing</a>
                </div>
            </div>
            <div class="footer-column">
                <div class="footer-column-title">Company</div>
                <div class="footer-links">
                    <a href="https://aicretesolutions.co.uk" class="footer-link" target="_blank">About</a>
                    <a href="https://aicretesolutions.co.uk" class="footer-link" target="_blank">Community</a>
                    <a href="https://aicretesolutions.co.uk" class="footer-link" target="_blank">Blog</a>
